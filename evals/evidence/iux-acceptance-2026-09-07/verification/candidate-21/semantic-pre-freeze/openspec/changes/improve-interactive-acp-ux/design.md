...
диагностика high, затем baseline high и medium. Пользователь явно одобрил
`new_scope`: узкий перенос ранее полученных high 78/78 после монотонного
ослабления первого critic predicate и изменения default concurrency.
Единственный owner условий и proof переноса — IUX-19 «Immutable evidence
manifest»; finalizer проверяет применимость, не создавая нового run или verdict.
Без этого proof baseline использует один candidate. Финальные
