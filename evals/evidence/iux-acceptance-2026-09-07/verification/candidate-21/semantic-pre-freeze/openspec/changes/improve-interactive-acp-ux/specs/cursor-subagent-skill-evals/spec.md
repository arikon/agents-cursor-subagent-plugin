...
он MUST проверить один candidate во всех eval inputs, кроме явно доказанного
переноса high по узкому исключению ниже; exact corpus-owned
...
Явно одобренное пользователем исключение MAY переносить ранее reproducibly
green high three-run series как `carried-forward`. Оно допускает только удаление
`changed.md` из `required_fragments` первого prompt-check `model-critic-delta`
и изменение default concurrency `8 → 12` с соответствующей строкой README.
Finalizer MUST проверить полные исходные high artifacts против их исходных
freeze и corpus, единственность указанного corpus delta, exact before/after
bytes двух evaluator files и неизменность всех остальных evaluator inputs,
skill, adapter и client. Изменение package payload допускается только как
следствие указанной строки README: оба payload hashes MUST быть вычислены
существующим package owner tree-hash algorithm из одинаковых hash-bound files
с единственной заменой README; пропуск или добавление file не может обходить
проверку исходного и текущего package hashes. `package_paths` является только
перечислением hash-domain данного proof, а package owner остаётся SSOT.
Source snapshots и proof используют те же
relative hashed regular-file references внутри общего bundle. Любое другое
изменение запрещает этот перенос; generic compatibility policy не вводится.
Текущие diagnostic и medium MUST по-прежнему использовать один frozen candidate
и concurrency 12. Исходный high сохраняет concurrency 4, candidate, counts и
verdicts; JSON proof и Markdown MUST явно обозначать `carried-forward` и обе
concurrency. Перенос доказывает применимость уже записанных high traces при
монотонном ослаблении oracle, но не является новым high run при concurrency 12.
...
кроме явно одобренного high carry-forward proof из «Immutable evidence manifest».
...
После diagnostic pass принимается high three-run series (свежая либо по
указанному carry-forward proof), затем свежая medium three-run series.
...
#### Scenario: Узкое ослабление critic predicate сохраняет применимость high
- **WHEN** переход удовлетворяет exact high carry-forward proof из
  «Immutable evidence manifest»
- **THEN** исходные high verdicts остаются неизменными и обозначаются
  `carried-forward`; текущие diagnostic и medium проходят свежие runs
