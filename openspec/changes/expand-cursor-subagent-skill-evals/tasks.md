...
- [ ] 1.1 Создать `evals/cursor-subagent-scenarios.v1.json` с точным closed
...
- [ ] 1.2 Создать единственный product module
...
- [ ] 1.3 Перевести `scripts/run-cursor-skill-eval.mjs` с `SCENARIOS`, outcome
...
- [ ] 2.1 Добавить в `tests/fixtures/fake-acp.mjs` explicit opt-in program mode
...
- [ ] 2.2 Реализовать «Scenario program driver и pure scenario oracle» в
...
- [ ] 2.3 Оставить `scripts/recording-mcp-proxy.mjs` единственным владельцем
...
- [ ] 3.0 Реализовать разделение «Разделённые eval lanes и evidence загрузки
...
- [ ] 3.1 Добавить ровно поле `manifest` в существующий evidence envelope и
  ...
  ordinal/module list.
- [ ] 3.2 Для programmed row outer создаёт exact workspace, принимает corpus и
...
- [ ] 3.3 Провести все client/model terminal branches через единый finalizer;
  ...
  всегда пишет child-result. Outer сверяет scenario/adapter/corpus/projection,
  выполняет свой cleanup, формирует final result и только затем как единственный
  publisher пишет immutable evidence. Проверить cleanup/publication precedence.
- [ ] 3.4 В `tests/release-e2e.test.mjs` передать `runLiveCanary` только outer
...
- [ ] 4.1 В существующем `tests/run-cursor-skill-eval.test.mjs` добавить один
...
- [ ] 4.2 Инструментировать ноль application child spawns, network и credential
  side effects; не создавать новый test file, дополнительную preparation stage
  либо nested suite.
- [ ] 4.3 Cheap injected spawn/env contract проверить исключение real-Codex,
...
- [ ] 4.4 Для tests с реальным filesystem/process I/O использовать отдельный
  `mkdtemp` и injected env без shared `process.env` mutation; inert no-I/O path
  strings оставить допустимыми.

## 5. Coverage и global owner gate

- [ ] 5.1 Добавить `scripts/cursor-eval-scenario.mjs` в `productSources`
  `scripts/run-node-tests.mjs`, не менять unit test-file set, и поведенчески
  покрыть module в существующем eval test file.
- [ ] 5.2 Registry хранит active/reference owner metadata без scenario IDs и
...
- [ ] 5.3 Проверить точные labels «Разделённые eval lanes и evidence загрузки skill», «Сценарный контракт поведения и authority-aware interaction», «Scenario program driver и pure scenario oracle», «Immutable evidence manifest» и «Cost-aware execution policy», полный MODIFIED delta и отсутствие второго semantic owner.

## 6. Acceptance и review

- [ ] 6.1 Запустить targeted tests, затем ровно по одному foreground `node
  ...
  points по `AGENTS.md`.
- [ ] 6.2 Запустить `openspec validate expand-cursor-subagent-skill-evals
  --strict`, `openspec validate --changes --strict` и global
  `node scripts/check-openspec-semantics.mjs`.
- [ ] 6.3 Провести полный critic review, передать весь verdict architect,
