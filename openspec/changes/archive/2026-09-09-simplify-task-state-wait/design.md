...
**Public-invariant index.** `SW-1` → «Публичный MCP tool contract», ссылка на preview из «Полное чтение terminal result»; `SW-2` → «Адресуемое ожидание состояния сессии»; `SW-3` → «Sparse wait and bounded progress», ссылка на неизменные bounds «Нормативные limits runtime»; `SW-4` → «Role-neutral mode and collaboration surface», согласованная ссылка из «Режимы Cursor и ACP callbacks»; `SW-5` → «Skill workflow делегирования»; `SW-6` → «Единая машина состояний runtime», «Изолированное состояние ходов и событий», «Ограниченный жизненный цикл ACP-процесса»; `SW-7` → «Сценарный контракт поведения и authority-aware interaction».
