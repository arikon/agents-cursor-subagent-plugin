...
1. Start with `cursor_delegate`. Choose `ask` for read-only work (including
   ...
   For that default launch, call `cursor_delegate` with only `cwd`, `mode`, and
   `prompt`; never probe or retry it with inferred launch fields.
...
5. At terminality, protocol completion does not prove semantic success: verify
   ...
   - Otherwise retain outcome/limitations, call `cursor_close_session` idempotently, then
     report. Do not substitute `cursor_cancel`: it is a recovery/diagnosis tool,
     not terminal cleanup for a completed or failed turn.
     Copy the retained `session_id` exactly; if close rejects it as unknown, compare
     it with the latest retained ID and retry only that corrected close.
     A completed turn in a tombstone is still completed; retain its provider ID
     for a later explicitly requested resume.

   A named later stage is one the user already announced: a later message, mode,
   plan, review, or wait-for-decision. A follow-up they did not announce is not
   named. Close only when work is complete, abandoned/cancelled, irrecoverably
...
6. On a named later stage, keep the live session. If the required mode differs,
   ...
   with `cursor_resume_session`. Its input is only `cwd`, the exact retained
   `cursor_session_id`, `mode`, and the requested `model`, `effort`, `fast`,
   ...
   delta line verbatim, including `-`/`+`. Explicit delta text may be supplied
   inline; do not require a preformatted diff block.
   - Same live runtime ID and unchanged baseline digest: replace the baseline body with
     `BASELINE_DIGEST=<retained digest>`, `CHANGED_PATHS=<bounded changed paths>`,
     `DELTA=<exact bounded delta>`. Include the user's current review requirements
     and exact outcome markers; omit only the unchanged baseline body.
   - After wrapper loss, tombstone, or any resume: the next prompt must include
     the full bounded baseline/snapshot plus that delta; never delta-only.
   If that evidence is missing, report unverifiable.
...
- **Rejected session call** (`unknown_session`, `unknown_turn`, `invalid_args`,
  ...
  retained ID. Correct only locally wrong/missing/malformed IDs in this Codex
  turn; never guess or launch provider work. A rejection using the exact
  retained IDs is not this repair. Suspected wrapper loss first requires correctly addressed observation
  of the retained session/turn; an address typo does not establish loss.
...
- **Between-turn mode error:** report normalized `error_code`. For `invalid_args`,
  repair only a malformed call with unambiguous already-authorized mode. For
   ...
   transition failure or observed tombstone enters irrecoverable recovery: retain
   the failure reason, report it and end the current Codex turn without another
   Cursor call. Require a new user decision for any later action; no
   retry/resume/replacement.
